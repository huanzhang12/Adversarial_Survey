...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        # load attack module
        if args['attack'] == "IterFGSM":
            print("IterFGSM")
            attack = Iter_FGSM(sess, model)
        else:
        ...
        targeted=targeted_flag, random_and_least_likely = True, predictor=attack.predict,
        ...
        print("all_true_ids = ", all_true_ids)
        ...
        attack.init_attack(sess, model, iter_num=args['iter_num'], targeted=targeted_flag, batch_size=attack_batch_size,
                           use_log = use_log)
        ...
        verbose_f = open(args['save'] + "/" + "_".join([args['dataset'], args['attack'],
                                                        str(targeted_flag), "verbose.txt"]), "w")
        aggre_f = open(args['save'] + "/" + "_".join([args['dataset'], args['attack'],
                                                      str(targeted_flag), "aggre.txt"]), "w")

        if targeted_flag == True:
            verbose_head_str = '\t'.join(['total', 'seq', 'id', 'epsilon', 'time', 'success', 'prev_class',
                                          'target', 'new_class', 'l0_distortion', 'l1_distortion', 'l2_distortion',
                                          'linf_distortion'])
        else:
            verbose_head_str = '\t'.join(['total', 'seq', 'id', 'epsilon', 'time', 'success', 'prev_class', 'new_class',
        ...
        for i in range(0, all_true_ids.size, attack_batch_size):
            ...
            adv, best_epsilon = attack.attack(inputs, targets, targeted=targeted_flag,
                                              initial_eps=args['initial_eps'], max_attempts=args['max_attempts'])
            ...
            for j in range(len(adv)):
                ...
                if targeted_flag == True:
                    verbose_str = '\t'.join(
                        [str(img_no), str(i+j), str(all_true_ids[i+j]), str(best_epsilon[j]), str(time_used_per_image),
                         str(success), str(original_class[-1]), str(np.argmax(targets[j])), str(adversarial_class[-1]),
                         str(l0_distortion), str(l1_distortion), str(l2_distortion), str(linf_distortion)])
                else:
                    verbose_str = '\t'.join(
                        [str(img_no), str(i+j), str(all_true_ids[i+j]), str(best_epsilon[j]), str(time_used_per_image),
                         str(success), str(original_class[-1]), str(adversarial_class[-1]), str(l0_distortion),
                         str(l1_distortion), str(l2_distortion), str(linf_distortion)])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--attack", choices=["IterFGSM"], default="IterFGSM")
    parser.add_argument("-u", "--untargeted", action='store_true')

    parser.add_argument("--initial_eps", type=float, default=0.01)
    ...
    parser.add_argument("-z", "--use_zvalue", action='store_true', default=True)
